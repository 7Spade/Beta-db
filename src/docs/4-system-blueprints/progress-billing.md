...
- **流程自動化**: 當滿足計價條件時，系統能自動或半自動地生成計價單。
...
### 階段二：進度計價單系統 (Progress Billing System)
- **位置**: 可作為「專案詳情」頁或「合約詳情」頁的一個新功能。
- **功能**:
    - 當專案進度達到付款排程中設定的條件時，系統應在儀表板或透過通知**提示**相關人員可以發起一筆新的計價。
    - 使用者啟動計價後，系統會引導其選擇本次計價要包含的**已完成任務**（數據來源於「專案管理」模組中已標記為「已完成」的任務）。
    - 系統根據所選任務，自動產生一張標準化的「計價單」，詳列本次完成的項目、數量、單價、金額，並與合約中的工料清單對應。
