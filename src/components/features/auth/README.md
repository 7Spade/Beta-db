...
此模組是整個應用程式安全和使用者身份管理的基石。它負責處理從使用者註冊、登入到個人資料管理的全流程。

## 架構設計

本模組遵循清晰的目錄結構，將不同關注點分離：

- **`views/`**: `login-view.tsx`, `register-view.tsx`, `profile-view.tsx` 等，這些是構成使用者介面的主要 React 元件。
