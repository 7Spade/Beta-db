      ...
      <TableBody>
        {receipts.map((receipt) => (
          <TableRow key={receipt.id}>
            ...
            <TableCell>
              {formatDate(receipt.requestDate instanceof Date ? receipt.requestDate : receipt.requestDate?.toDate())}
            </TableCell>
            <TableCell>
              <ContractStatusBadge status={receipt.status as '啟用中' | '已完成' | '暫停中' | '已終止'} />
            </TableCell>
            <TableCell>
              {receipt.receivedDate 
                ? formatDate(receipt.receivedDate instanceof Date ? receipt.receivedDate : receipt.receivedDate?.toDate())
                : '未收款'
              }
            </TableCell>
